...
export class TodoItemResolver implements ResolverInterface<TodoItem> {
    ...
    async TodoItem(
        ...
        return await Prefill.Instance.Items.find(
    ...
    @Mutation(() => TodoItem)
    async Add(
        @Arg("TodoItem") todoItemInput: TodoItemInput
    ): Promise<TodoItem> {
    ...
    private CreateTodoSchema<T extends TodoItem | TodoItemInput>(
        todoItem: T
    ): ITodoSchema {
        return <ITodoSchema>{
            Id: todoItem.Id,
    ...
    async Update(
        ...
        const item:
            | TodoItem
            | undefined = await Prefill.Instance.Items.find(
            (x) => x.Id === todoItemInput.Id
        );
    ...
    async Remove(@Arg("Id") id: string): Promise<boolean> {
        const index = Prefill.Instance.Items.findIndex(
            (x) => x.Id === id
        );
        if (index < 0) {
            return false;
